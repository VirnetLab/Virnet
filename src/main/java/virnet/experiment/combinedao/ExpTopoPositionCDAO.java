...
import virnet.experiment.dao.ExpTopoPositionDAO;
import virnet.experiment.entity.ExpTopoPosition;

public class ExpTopoPositionCDAO {
	
	private ExpTopoPositionDAO tpDAO = new ExpTopoPositionDAO();
	
	public boolean edit(Integer expTopoId,String position){
		//position字符串 设备间用逗号相隔    X，Y以空格相隔
		
		String grid[]=position.split(" |,");
		
		@SuppressWarnings("unchecked")
		List<ExpTopoPosition> plist = this.tpDAO.getListByProperty("expTopoId", expTopoId); 
		//删除以往记录
		int i = 0;
		while(i!=plist.size()){
			this.tpDAO.delete(plist.get(i));
			i++;
		}
		//重写
		i = 0;
		...
		while(i<grid.length){
			...
			ExpTopoPosition newposition = new ExpTopoPosition();
			newposition.setExpTopoId(expTopoId);
			...
			i++;
			deviceOrder++;
			flag = this.tpDAO.add(newposition);
			if(flag == false)
				success = false;
		}
		return success;
	...
	public String position(Integer expTopoId){
		String position = "";
		int deviceOrder=1;
		while(true){
			String para[] ={"expTopoId",""+expTopoId,"deviceOrder",""+deviceOrder}; 
			ExpTopoPosition tp = (ExpTopoPosition) this.tpDAO.getByNProperty(para);
			//如果查询结果不存在，即遍历了所有设备
			if(tp == null)    break;
			position = position + tp.getGridX() + " " + tp.getGridY() + ",";
			deviceOrder++;
		}
		//将末尾的逗号去除
		return position.substring(0, position.length()-1);
